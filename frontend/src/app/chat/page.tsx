      ...
      <div className="max-w-3xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-400">{t("chat.subtitle")}</p>
          </div>
          {studies.length > 0 && (
            <select
              value={selectedStudy}
        ...
        <div className="flex-1 bg-white border border-sky-100 rounded-2xl overflow-y-auto p-6 space-y-4 shadow-sm">
          {messages.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 font-semibold">{t("chat.welcome1")}</p>
              <p className="text-gray-400 text-sm mt-1">{t("chat.welcome2")}</p>
              <div className="flex flex-wrap gap-2 justify-center mt-6">
                {[t("chat.suggestion1"), t("chat.suggestion2"), t("chat.suggestion3")].map((q) => (
                  <button
                    key={q}
                    onClick={() => setInput(q)}
                    className="px-3 py-1.5 bg-sky-50 hover:bg-sky-100 text-sky-600 border border-sky-100 rounded-full text-xs transition-colors font-medium cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
        ...
        <form onSubmit={handleSend} className="mt-4 flex gap-3">
          ...
          <button
            ...
            className="px-5 py-3 bg-gradient-to-r from-sky-400 to-cyan-500 hover:from-sky-500 hover:to-cyan-600 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow-md cursor-pointer"
          >
            {t("chat.send")}
          </button>
        </form>
